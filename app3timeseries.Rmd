...
```
There is only one significant pic at level one on ACP and one pic at level 7 on PACF.

```{r}
r2t <- rt^2
acf(r2t, main="ACF of squared returns (r²t)", lag.max=40)
```
This graph indicates pics at lags 1, 2 and 3. It confirms the presence volatitly on the series.


SO P = 0 or 1, d = 1 and q = 0 or 7.
